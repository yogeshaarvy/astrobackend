...
import {
  addEditBlogsBanner,
  fetchBlogsBanner
} from '@/redux/slices/blogsbannerSlice';
...
const Blogs: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(fetchBlogsBanner())
      .unwrap()
      .then((response) => {
        if (response?.blogsbannerData?.[0]) {
          const prevData = response.blogsbannerData[0];
          setFormData((prev) => ({
            ...prev,

            heading: prevData.heading || '',
            subheading: prevData.subheading || '',
            background_image: prevData.background_image || '',
            image1: prevData.image1 || '',
            image2: prevData.image2 || ''
          }));
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = await dispatch(addEditBlogsBanner(formData)).unwrap();
      ...
      <form className="grid grid-cols-1 gap-6 p-10" onSubmit={handleSubmit}>
        <div className="flex gap-10">
          ...
          <div className="flex flex-col">
            <label htmlFor="image2" className={labelClasses}>
              Image2
            </label>
            <div className="space-y-2">
              <input

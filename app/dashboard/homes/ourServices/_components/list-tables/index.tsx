...
export default function OurServicesListTable({
  data,
  totalData,
  handleSearch
}: {
  ...
  const columns: ColumnDef<IOurServices>[] = [
    {
...

    {
      ...
      cell: ({ row }) => {
        const handleToggle = async (checked: boolean) => {
          ...
          try {
            dispatch(updateOurServicesListData(updatedData));
            const result = await dispatch(
              addEditOurServicesList(row.original._id || null)
            ).unwrap();

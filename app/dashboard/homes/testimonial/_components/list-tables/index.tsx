...
export default function TestimonialListTable({
  ...
  const columns: ColumnDef<IHomeTestimonial>[] = [
    {
...

    {
      ...
      cell: ({ row }) => {
        const handleToggle = async (checked: boolean) => {
          ...
          try {
            dispatch(updateTestimonialListData(updatedData));
            const result = await dispatch(
              addEditTestimonialList(row.original._id || null)
            ).unwrap();

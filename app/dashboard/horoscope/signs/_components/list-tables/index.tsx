...
export default function HoroscopeListTable({
  data,
  totalData,
  handleSearch
}: {
  ...
  const columns: ColumnDef<IHoroscope>[] = [
    {
...

    {
      ...
      cell: ({ row }) => {
        const handleToggle = async (checked: boolean) => {
          ...
          try {
            dispatch(updateHoroscopeListData(updatedData));
            const result = await dispatch(
              addEditHoroscopeList(row.original._id || null)
            ).unwrap();

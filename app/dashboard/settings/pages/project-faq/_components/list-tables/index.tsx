...
export default function ProjectFaqListTable({
  data,
  totalData,
  handleSearch
}: {
  ...
  const columns: ColumnDef<IProjectfaq>[] = [
    {
...

    {
      ...
      cell: ({ row }) => {
        const handleToggle = async (checked: boolean) => {
          ...
          try {
            dispatch(updateProjectfaqListData(updatedData));
            const result = await dispatch(
              addEditProjectfaqList(row.original._id || null)
            ).unwrap();

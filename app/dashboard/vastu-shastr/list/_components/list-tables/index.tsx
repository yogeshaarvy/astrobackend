...
export default function VastushastrListTable({
  data,
  totalData,
  handleSearch
}: {
  data: IVastushastrList[];
  totalData: number;
  handleSearch: any;
}) {
  ...
  const columns: ColumnDef<IVastushastrList>[] = [
    {
...

    {
      ...
      cell: ({ row }) => {
        const handleToggle = async (checked: boolean) => {
          ...
          try {
            dispatch(updateVastushastrListData(updatedData));
            const result = await dispatch(
              addEditVastushastrList(row.original._id || null)
            ).unwrap();

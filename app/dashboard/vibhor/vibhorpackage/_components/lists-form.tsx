...
export default function VibhorPackageForm() {
  ...
  console.log('this is the response of package data', packageData);
  const form = useForm<IVibhorPackage>({});
  ...
  const handleSubmit = (e: any) => {
    ...
    if (!packageData.hasQuestions) {
      toast.error('Please select whether this package has questions');
      return;
    }

    // Validate questionsCount only if hasQuestions is 'yes'
    if (packageData.hasQuestions === 'yes' && !packageData.questionsCount) {
      toast.error('Please provide the number of questions');
      return;
    }
  ...
  const handleDropdownChange = (e: any) => {
    ...
    // If hasQuestions changes to 'no', clear questionsCount
    if (name === 'hasQuestions' && value === 'no') {
      dispatch(
        updateVibhorPackageData({
          [name]: value,
          questionsCount: null
          ...
          <div className="space-y-8">
            <Form {...form}>
              ...
              >
                ...
                {/* Questions Section - Updated to match schema */}
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <CustomDropdown
                      label="Has Questions *"
                      ...
                      value={packageData?.hasQuestions || ''}
                      onChange={handleDropdownChange}
                    />
                    <p className="text-sm text-gray-500">
                      Select whether this package includes questions
                    </p>
                  </div>

                  {/* Show questions count only if hasQuestions is 'yes' */}
                  {packageData?.hasQuestions === 'yes' && (
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        value={packageData?.questionsCount || ''}
                        onChange={handleInputChange}
                        required
                        min="1"
                      />
                      <p className="text-sm text-gray-500">
                        Enter the total number of questions in this package
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="rounded-lg bg-blue-50 p-4">
                    <p className="font-medium text-blue-700">
                      ...
                      {packageData.hasQuestions === 'yes'
                        ...
                        : 'Without Questions'}
                      {packageData.hasQuestions === 'yes' &&
                        packageData.questionsCount && (
                          <span className="text-blue-600">
                            {' '}
                            ({packageData.questionsCount} question
                            {packageData.questionsCount > 1 ? 's' : ''})
                          </span>
                        )}
                    </p>
                  </div>

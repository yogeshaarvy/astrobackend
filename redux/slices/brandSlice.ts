...
export const fetchBrandList = createAsyncThunk<
    any,
    { page?: number; pageSize?: number; keyword?: string, field: string; status: string; exportData: string } | void,
    { state: RootState }
>(
    'brand/fetchBrandList',
    async (input, { dispatch, rejectWithValue, getState }) => {
        try {
            const { page, pageSize, keyword, field, status, exportData } = input || {};

            dispatch(fetchBrandStart());
            const response = await fetchApi(`/brands/all?page=${page || 1}&limit=${pageSize || 10}&text=${keyword || ""}&field=${field || ""}&active=${status || ""}&export=${exportData}`, { method: 'GET' });
...
>('brand/add', async (entityId, { dispatch, rejectWithValue, getState }) => {
    try {
        ...
        let response;
        if (!entityId) {
            response = await fetchApi('/brands/new', {
                method: 'POST',
                body: formData
            });
        } else {
            response = await fetchApi(`/brands/update/${entityId}`, {
                method: 'PUT',
                body: formData
            });
        }
...
>(
    ...
    async (entityId, { dispatch, rejectWithValue, getState }) => {
        try {
            ...
            const response = await fetchApi(`/brands/${entityId}`, { method: "GET" });
            if (response?.success) {
...
export const deleteBrand = createAsyncThunk<any, string, { state: RootState }>(
    ...
    async (id, { dispatch }) => {
        ...
        try {
            const response = await fetchApi(`/brands/delete/${id}`, { method: 'DELETE' });
            if (response.success) {

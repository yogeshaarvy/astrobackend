...
export const fetchCategoryList = createAsyncThunk<
    any,
    { page?: number; pageSize?: number; keyword?: string, field: string; status: string, exportData: string, entityId: string } | void,
    { state: RootState }
>(
    'category/fetchCategoryList',
    async (input, { dispatch, rejectWithValue, getState }) => {
        try {
            const { page, pageSize, keyword, field, status, exportData, entityId } = input || {};
            dispatch(fetchCategoryStart());
            const response = await fetchApi(`/category/all?page=${page || 1}&limit=${pageSize || 10}&text=${keyword || ""}&field=${field || ""}&active=${status || ""}&export=${exportData}&entityId=${entityId || ''}`, { method: 'GET' });
...
>('category/add', async (entityId, { dispatch, rejectWithValue, getState }) => {
    try {
        ...
        let response;
        if (!entityId) {
            response = await fetchApi('/category/new', {
                method: 'POST',
                body: formData
            });
        } else {
            response = await fetchApi(`/category/update/${entityId}`, {
                method: 'PUT',
                body: formData
            });
        }
...
>(
    ...
    async (entityId, { dispatch, rejectWithValue, getState }) => {
        try {
            ...
            const response = await fetchApi(`/category/${entityId}`, { method: "GET" });
            if (response?.success) {
...
export const deleteCategory = createAsyncThunk<any, string, { state: RootState }>(
    'category/delete',
    async (id, { dispatch }) => {
        dispatch(deleteCategoryStart());
        try {
            const response = await fetchApi(`/category/delete/${id}`, { method: 'DELETE' });
            if (response.success) {
...
const categorySlice = createSlice({
    ...
    reducers: {
        ...
        updateCategoryData(state, action) {
            ...
            state.singleCategoryState.data = { ...oldData, ...action.payload };
        },

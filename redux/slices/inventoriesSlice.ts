  ...
  async (input, { dispatch, rejectWithValue, getState }) => {
    try {
      ...
      const response = await fetchApi(
        `/inventories/all?page=${page || 1}&limit=${
          pageSize || 10
        }&productype=${producttype || ''}&stock_management=${
          stock_management || ''
        }&productId=${productId || ''}&variantId=${variantId || ''}&export=${
          exportData || ''
...
export const deleteInventory = createAsyncThunk<
  ...
  string,
  { state: RootState }
>('inventory/delete', async (id, { dispatch }) => {
  ...
  try {
    ...
    dispatch(
      deleteInventoryFailure(error.message || 'Failed to delete inventory')
    );
...
>(
  ...
  async (input, { dispatch, rejectWithValue, getState }) => {
    try {
      ...
      const response = await fetchApi(
        `/inventories/products?page=${page || 1}&limit=${
          pageSize || 10
        }&keyword=${
          keyword || ''
        }&exportData=${exportData}&selectedProductIds=${
          selectedProductIds || ''

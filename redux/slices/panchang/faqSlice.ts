...
>(
  ...
  async (entityId, { dispatch, rejectWithValue, getState }) => {
    try {
      ...
      let response;
      if (!entityId) {
        response = await fetchApi('/panchang/faq/create', {
          method: 'POST',
          body: formData
        });
      } else {
        response = await fetchApi(`/panchang/faq/update/${entityId}`, {
          method: 'PUT',
          body: formData
        });
      }
  ...
  async (input, { dispatch, rejectWithValue, getState }) => {
    try {
      ...
      const response = await fetchApi(
        `/panchang/faq/all?page=${page || 1}&limit=${pageSize || 5}&field=${
          field || ''
...
export const fetchSingleListFaq = createAsyncThunk<
  ...
  { state: RootState }
>(
  'panchang/getSingleList',
  async (entityId, { dispatch, rejectWithValue, getState }) => {
    try {
      dispatch(fetchSingleListFaqStart());
      const response = await fetchApi(`/panchang/faq/get/${entityId}`, {
        method: 'GET'
      });
      if (response?.success) {
...
export const deleteFaq = createAsyncThunk<any, string, { state: RootState }>(
  ...
  async (id, { dispatch }) => {
    ...
    try {
      const response = await fetchApi(`/panchang/faq/delete/${id}`, {
        method: 'DELETE'
      });
      if (response.success) {
...
const panchangFaqSlice = createSlice({
  name: 'panchangFaq',
  initialState,
  reducers: {
...
export const {
  ...
  deleteFaqSuccess,
  deleteFaqFailure
} = panchangFaqSlice.actions;
export default panchangFaqSlice.reducer;
